...
This article describes limitations and known issues of BlobFuse2.

## Chmod operations in premium performance block blob accounts

Premium performance block blob accounts don't support access control lists (ACLs). BlobFuse2 returns success for `chmod` operations in those types of accounts. 
...
By default, BlobFuse2 will send logs to Syslog. The default settings will, in some cases, log relevant file paths to syslog. If this is sensitive information, turn off logging or set log-level to `log_err`.  

## Unsupported file system operations

The following file system operations are not supported in BlobFuse2:
...
The following file system workflows aren't supported in BlobFuse2:
...
The following file system operations have altered behavior in BlobFuse2
...
- For storage accounts that have a flat namespace, when data is uploaded through other means, BlobFuse2 expects special directory marker files to exist in container. For example, if you have a blob `A/B/c.txt` then special marker files shall exist for `A` and `A/B`. To overcome this requirement, BlobFuse2 uses ListBlob aoi instead of GetBlobProperties api for `ls` operation though `ListBlob` is more expensive.
