...
- Build highly available applications that programmatically interact with the MQTT broker for reliable communication with Azure IoT Operations services. For example, the OPC UA connector enables you to control connected OPC UA servers by sending commands through the MQTT broker. The protocol compiler tool in the SDK lets you generate client code from DTDL models to simplify the development of such applications. The v2 version of the protocol compiler uses [Web of Things (WoT)](https://www.w3.org/TR/wot-thing-description11/) Thing model files as input To learn more, see:
    ...
    > [!TIP]
    > The [TestThing](https://github.com/Azure/iot-operations-sdks/tree/johndo/sample-TestThing-Counter/dotnet/samples/Protocol/TestThing) folder contains a example of a .NET library generated by the protocol compiler v2 from a WoT Thing [model file](https://github.com/Azure/iot-operations-sdks/blob/johndo/sample-TestThing-Counter/eng/test/schema-samples/Counter.TM.json). You can use this library as a starting point for building your own applications that interact with Azure IoT Operations services through the MQTT broker.
