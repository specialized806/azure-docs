...
The following procedure creates a virtual network with a resource subnet and an Azure Bastion host.
...
1. In the **Azure Bastion** section of the **Security** tab, select **Enable Azure Bastion**.
...
1. Enter or select the following information in **Azure Bastion**:
    ...
    | Azure Bastion SKU | Select **Developer**. |

    > [!NOTE]
    > The Developer SKU for Azure Bastion is free and doesn't require an **AzureBastionSubnet**. The subnet is created automatically. For more information, see [Quickstart: Deploy Azure Bastion - Developer SKU](../bastion/quickstart-developer-sku.md).
...
In this section, you create a virtual network with a resource subnet and an Azure Bastion host.
