...
Azure Migrate now supports migrating your Generation 2 Virtual Machines to Azure Virtual Machines with Trusted Launch. Trusted Launch uses UEFI-based Secure Boot and a virtual Trusted Platform Module (vTPM) to establish a trusted boot chain. This ensures that only approved and signed components load during startup, reducing the risk from bootkits, rootkits, and other low-level malware.
