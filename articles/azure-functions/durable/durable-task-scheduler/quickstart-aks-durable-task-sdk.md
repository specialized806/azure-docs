---
title: Host a Durable Task SDK app on Azure Kubernetes Service
description: Learn how to deploy durable orchestrations in your Durable Task SDK app to Azure Kubernetes Service for full control over scaling, networking, and operations.
author: hhunter-ms
...
# Host a Durable Task SDK app on Azure Kubernetes Service
...
## Add health endpoints to worker
...
1. **Workload identity label**: Pods must have `azure.workload.identity/use: "true"` label
1. **Service account annotation**: Must include `azure.workload.identity/client-id` annotation
1. **ConfigMap not applied**: Run `kubectl get configmap -n durable-system` to verify
1. **Variable substitution**: Use `envsubst` or Helm for variable replacement before applying manifests

## Next steps

- [Host a Durable Task SDK app on Azure Container Apps](quickstart-container-apps-durable-task-sdk.md)
- [Scaling using Durable Task SDKs](durable-task-scheduler-auto-scaling-aks.md)
