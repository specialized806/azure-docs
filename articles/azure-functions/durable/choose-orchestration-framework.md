...
description: Learn how your hosting platform determines which Durable Task programming model to use — Durable Functions for Azure Functions or the standalone Durable Task SDKs for self-hosted scenarios.
author: cgillum
ms.author: cgillum
...
> [!NOTE]
> Azure App Service and Azure Container Apps can both host the Azure Functions runtime, either through [fully managed Azure Functions integration](../functions-scale.md#overview-of-plans) or by deploying the Functions runtime directly. Because of this, both platforms support either programming model. For more information on Azure Functions hosting options, see [Azure Functions hosting plans](../functions-scale.md).
...
When choosing between the two programming models, consider the following additional factors:
...
All Durable Task SDKs are open source and available on GitHub. However, some SDKs are not officially supported by Microsoft or are still in experimental stages. The following SDKs are currently unsupported:

### Durable Task Framework (Legacy)

The [Durable Task Framework](https://github.com/Azure/durabletask) (DTFx) is an older, open-source .NET Durable Task library. While it provides similar orchestration primitives, it predates the modern Durable Task SDKs and doesn't include official Microsoft support or the latest features. It also requires users to manage hosting, operational infrastructure, and long-term maintenance themselves.
...
The [Durable Task SDK for Go](https://github.com/Azure/durabletask-go) is a community-supported, open-source library that enables durable orchestration capabilities in Go applications. It is currently in experimental stages, does not work with any of the supported Durable Task state storage backends, and is not recommended for production use.
