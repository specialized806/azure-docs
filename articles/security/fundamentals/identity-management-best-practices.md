...
**Detail**: The default Microsoft Entra Connect filtering only excludes the built-in Administrator account (RID 500). To protect other highly privileged accounts such as Domain Admins and Enterprise Admins, use [organizational unit (OU) filtering](/entra/identity/hybrid/connect/how-to-connect-sync-configure-filtering#organizational-unitbased-filtering) to place these accounts in dedicated OUs that are excluded from synchronization, or use [attribute-based filtering](/entra/identity/hybrid/connect/how-to-connect-sync-configure-filtering#attribute-based-filtering) to exclude accounts with specific group memberships. This configuration mitigates the risk of adversaries pivoting from cloud to on-premises assets (which could create a major incident).
