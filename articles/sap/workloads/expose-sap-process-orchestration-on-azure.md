...
Security concerns require [firewalls](../../firewall/features.md) for lower-level protocols and [WAFs](../../web-application-firewall/overview.md) to address HTTP-based traffic with [Transport Layer Security (TLS)](https://en.wikipedia.org/wiki/Transport_Layer_Security). To be effective, TLS sessions need to be terminated at the WAF level. To support zero-trust approaches, we recommend that you [re-encrypt](../../application-gateway/ssl-overview.md) afterward to provide end-to-end encryption.
