---
title: Azure VMs high availability for SAP NetWeaver on SLES multi-SID
description: Learn how to implement multi-SID high-availability for SAP NetWeaver on SUSE Linux Enterprise Server for SAP applications.
...
To achieve high availability, SAP NetWeaver requires highly available NFS shares. In this example, we assume the SAP NFS shares are hosted on highly available [NFS file server](./high-availability-guide-suse-nfs.md), which multiple SAP systems can use. Or the shares are deployed on [Azure NetApp Files NFS volumes](../../azure-netapp-files/azure-netapp-files-create-volumes.md).
...
The referenced articles guide you through the steps to prepare the necessary infrastructures, build the cluster, and prepare the OS for running the SAP application.

> [!TIP]
> Always test the failover functionality of the cluster after the first system is deployed and before adding more SAP SIDs to the cluster. Testing is crucial so that you know that the cluster functionality works before adding the complexity of extra SAP systems to the cluster.

### Deploy multiple SAP systems in the cluster

In this example, we assume that system **NW1** was already deployed in the cluster. We're also demonstrating how to deploy SAP systems **NW2** and **NW3** in the cluster.
...
> [!IMPORTANT]
> Before following the instructions to deploy more than one SAP systems in the cluster, follow the instructions to deploy the first SAP system in the cluster, as there are steps that are only necessary during the first system deployment.
...
* The cluster failover functionality is tested.
* The NFS shares for all SAP systems are deployed.

### Prepare for SAP NetWeaver Installation

1. Add configuration for the newly deployed system (**NW2** and **NW3**) to the existing Azure Load Balancer, following the instructions [configure Azure Load Balancer manually via Azure portal](./high-availability-guide-suse-netapp-files.md#configure-azure-load-balancer). Adjust the IP addresses, health probe ports, load-balancing rules for your configuration.

1. **[A]** Configure name resolution for the other SAP systems. You can either use DNS server or modify `/etc/hosts` on all nodes. This example shows how to use the `/etc/hosts` file. Adapt the IP addresses and the host names to your environment.
...
1. **[A]** Create the shared directories for SAP systems **NW2** and **NW3** that you're deploying to the cluster.
...
1. **[A]** Configure `autofs` to mount the `/sapmnt/SID` and `/usr/sap/SID/SYS` file systems for the other SAP systems that you're deploying to the cluster. In this example **NW2** and **NW3**.

   Update file `/etc/auto.direct` with the file systems for the other SAP systems that you're deploying to the cluster.
...
1. Create the virtual IP and health probe cluster resources for the ASCS instance of the other SAP system you're deploying to the cluster. The example shown is for **NW2** and **NW3** ASCS, using highly available NFS server.
   ...
   >
   > The change requires brief downtime.
   ...
   As you creating the resources they may be assigned to different cluster resources. When you group them, they're migrated to one of the cluster nodes. Make sure the cluster status is ok and that all resources are started. It isn't important on which node the resources are running.
...
1. **[1]** Create a virtual IP and health-probe cluster resources for the ERS instance of the other SAP system you're deploying to the cluster. The example shown is for **NW2** and **NW3** ERS, using highly available NFS server.
   ...
   As you creating the resources they may be assigned to different cluster nodes. When you group them, they're migrated to one of the cluster nodes. Make sure the cluster status is ok and that all resources are started.
...
4. **[2]** Install SAP NetWeaver ERS
   ...
   > [!NOTE]
   > Use SWPM SP 20 PL 05 or later. Lower versions don't set the permissions correctly causing the installation to fail.
...
5. **[1]** Adapt the ASCS/SCS and ERS instance profiles for one or more newly installed SAP systems. The example shown is for **NW2**. You need to adapt the ASCS/SCS and ERS profiles for all SAP instances added to the cluster.
...
7. Add the ASCS and ERS SAP services for the newly installed SAP system to the `sapservice` file. The example shown is for SAP systems **NW2** and **NW3**.
...
8. **[A]** Disabling `systemd` services of the ASCS and ERS SAP instance. This step is only applicable if systemd manages the SAP startup framework as per SAP Note [3115048](https://me.sap.com/notes/3115048).
...
The following picture shows how the resources would look like in the High Availability Web Konsole (HAWK), with the resources for SAP system **NW2** expanded.
...
* Installing extra SAP application instances
...
Always read the SUSE best practices guides and perform all tests as required. The tests presented are in a two-node, multi-SID cluster with three SAP systems installed.

1. Test HAGetFailoverConfig and HACheckFailoverConfig

   Run the following commands as \<sapsid\>adm on the node where the ASCS instance is currently running. If there are dashes in the hostname, the commands might fail with **FAIL: Insufficient memory** (a known issue). SUSE is to correct it in the sap-suse-cluster-connector package.
...
1. Manually migrate the ASCS instance. The example shows migrating the ASCS instance for SAP system NW2.
