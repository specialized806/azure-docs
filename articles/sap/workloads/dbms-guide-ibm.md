...
With Microsoft Azure, you can migrate your existing SAP application running on IBM Db2 for Linux, UNIX, and Windows (LUW) to Azure virtual machines. With SAP on IBM Db2 for LUW, administrators and developers can still use the same development and administration tools, which are available on-premises. General information about running SAP Business Suite on IBM Db2 for LUW is available via the SAP Community Network (SCN) in [SAP on IBM Db2 for Linux, UNIX, and Windows](https://www.sap.com/community/topic/db2-for-linux-unix-and-windows.html).
...
SAP on IBM Db2 for LUW on Microsoft Azure Virtual Machine Services is supported as of Db2 version 10.5. For information about supported SAP products and Azure VM(Virtual Machines) types, refer to SAP Note [1928533].
...
Remote shared volumes like the Azure services in the listed scenarios **aren't** supported for Db2 database files:
...
For performance considerations, also refer to the chapter "Data Safety and Performance Considerations for Database Directories" in SAP installation guides.

Alternatively, you can use Windows Storage Pools available in Windows Server 2012 and later as described [Considerations for Azure Virtual Machines DBMS deployment for SAP workload](dbms-guide-general.md). On Linux, you can use LVM or MDADM to create one large logical device over multiple disks.
...
IBM Db2 for SAP NetWeaver Applications is supported on any VM type listed in SAP support note [1928533]. Recommended VM families for running IBM Db2 database are Esd_v4/Eas_v4/Es_v3 and M/M_v2-series for large multi-TB databases. Enabling the M-series Write Accelerator can improve write performance on the IBM Db2 transaction log disk.
...
This criterion is for a database size of 50 GB - 200 GB (example: Solution Manager).

| VM Size / Examples |Db2 mount point |Azure Premium Disk |# of Disks |IOPS |Throughput [MB/s] |Size [GB] |Burst IOPS |Burst Throughput [GB] | Stripe size | Caching |
| --- | --- | --- | :---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
...
# [Small SAP system](#tab/small)

This criterion is for a database size of 200 GB - 750 GB (example: small Business Suite).
...
# [Medium SAP system](#tab/medium)

This criterion is for a database size of 500 GB - 1000 GB (example: small Business Suite).
...
# [Large SAP system](#tab/large)

This criterion is for a database size of 750 GB - 2000 GB (example: Business Suite).
...
# [Large multi-TB SAP system](#tab/large-multi)

This criterion is for a database size of 2 TB and more (example: Global Business Suite).

Especially for larger systems, it's important to evaluate the infrastructure that the system is currently running on. It's also necessary to review the resource consumption data of those systems to find the best match of Azure compute and storage infrastructure and configuration.

| VM Name / Size |Db2 mount point |Azure Premium Disk |# of Disks |IOPS |Throughput [MB/s] |Size [GB] |Burst IOPS |Burst Throughput [GB] | Stripe size | Caching |
| --- | --- | --- | :---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
...
![A diagram of the IBM Db2 storage configuration using Azure ANF volumes.](./media/dbms_guide_ibm/anf-configuration-example.png)
...
As of mount options, mounting those volumes could look like (replace `<SID>` and `<sid>` by the SID of your SAP system):
...
> [!NOTE]
> The mount parameters `hard` and `sync` are **required**.

### Backup and restore

The backup/restore functionality for IBM Db2 for LUW is supported in the same way as on standard Windows Server Operating Systems and Hyper-V. Make sure that you have a valid database backup strategy in place.
...
> [!NOTE]
> Db2 on Windows doesn't support the Windows VSS technology. As a result, the application consistent VM backup of Azure Backup Service can't be used for VMs the Db2 DBMS is deployed in.
...
If the IOPS or I/O throughput of a single Azure VHD isn't sufficient, you can use LVM (Logical Volume Manager) or MDADM. These tools allow you to create one large logical device over multiple disks as described in [Considerations for Azure Virtual Machines DBMS deployment for SAP workload](dbms-guide-general.md).
