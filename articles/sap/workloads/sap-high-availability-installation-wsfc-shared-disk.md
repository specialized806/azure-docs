---
title: Install SAP NetWeaver HA on a Windows failover cluster with shared disk for an SAP ASCS/SCS instance in Azure
...
> [!IMPORTANT]
> If you use SIOS to present shared disk, don't place your page file on the SIOS DataKeeper mirrored volumes. You can leave your page file on the temporary drive **D:** of an Azure VM, which is the default. If it's not already there, move the Windows page file to drive **D:** of your Azure VM.
...
1. If using the new SAP Enqueue Replication Server 2, which is also clustered instance, then you need to reserve in DNS a virtual host name for ERS2 as well.
...
1. To define the IP address assigned to the virtual host name in **DNS Manager**:

   1. Expand your zone.

   1. Open the entry that you need to define the IP address.

   ![A screenshot of the Windows DNS Manager with multiple entries in a forward lookup zone.][sap-ha-guide-figure-3047]
...
1. Execute the first cluster node option on cluster node A. Select:
   ...
   * **ABAP+Java system**: **ASCS** instance number **00** and **SCS** instance number **01**
   ...
   > [!IMPORTANT]
   > The configuration in the Azure internal load balancer load balancing rules (if using a Basic SKU) and the selected SAP instance numbers must match.

1. Follow the SAP described installation procedure. Make sure in the start installation option under "First Cluster Node" that "Cluster Shared Disk" is selected as a configuration option.
...
If you have Enqueue Replication Server 1, add the SAP profile parameter `enque/encni/set_so_keepalive`. The profile parameter prevents connections between SAP work processes and the enqueue server from closing when they're idle for too long. The SAP parameter isn't required for ERS2.
...
1. To apply the SAP profile parameter changes, restart the SAP ASCS/SCS instance.

### Add a probe port

To make the entire cluster configuration work with Azure Load Balancer, use the internal load balancer's probe functionality. The Azure internal load balancer usually distributes the incoming workload equally between participating virtual machines (VMs).

However, the workload distribution doesn't work in some cluster configurations because only one instance is active. The other instance is passive and can’t accept any of the workload. A probe functionality helps when the Azure internal load balancer detects which instance is active, and only target the active instance.

> [!IMPORTANT]
> In this example configuration, the **ProbePort** is set to 620**Nr**. For SAP ASCS instance with number 00,** it's 620**00**. You need to adjust the configuration to match your SAP instance numbers and your SAP SID.
...
If using ERS2, you must open the firewall port for the ERS2 probe port.
...
Install the Primary Application Server (PAS) instance \<SID\>-di-0 on the VM that you designate to host the PAS. There are no dependencies on Azure. If using SIOS, there are no DataKeeper-specific settings.

## Install the SAP Additional Application Server

Install an SAP Additional Application Server (AAS) on all the VMs designated to host an SAP Application Server instance.
...
1. Restart cluster node A within the Windows guest operating system. Doing so initiates an automatic failover of the SAP \<SID\> cluster group from node A to node B.
1. Restart cluster node A from the Azure portal. Doing so initiates an automatic failover of the SAP \<SID\> cluster group from node A to node B.
1. Restart cluster node A by using Azure PowerShell. Doing so initiates an automatic failover of the SAP \<SID\> cluster group from node A to node B.

1. After failover, verify that the SAP \<SID\> cluster group is running on cluster node B.

   ![A screenshot of the Windows Failover Cluster Manager with an SAP cluster configured as a role.][sap-ha-guide-figure-5002]
   ...
   ![A screenshot of SIOS DataKeeper replicating the local volume from cluster node B to cluster node A.][sap-ha-guide-figure-5003]
