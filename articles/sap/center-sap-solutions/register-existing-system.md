---
title: Register an existing SAP system
...
- Grant access to Azure Storage accounts, Azure Resource Manager, and Microsoft Entra services from the virtual network where the SAP system exists. Use one of these options:
    ...
    - Region-specific IP addresses for Azure Storage, Azure Resource Manager (ARM), and Microsoft Entra ID must be in the allow list.
- For Windows system, ACSS requires outbound connectivity to `www.microsoft.com`.
    - ACSS runs PowerShell scripts through a VM extension to provide various capabilities. PowerShell scripts are Microsoft-signed. On the Windows OS, the script signature is validated and for the validation to be successful the URL must be allow-listed. Otherwise, functionalities in ACSS don't work as expected.
    - If you use a firewall, you can add an outbound rule to an endpoint `www.microsoft.com` port 80
- ACSS deploys a **managed storage account** into your subscription, for each SAP system being registered. Choose the [**network access**](#managed-storage-account-network-access-settings) setting for the storage account.
    - If you choose network access from specific Virtual Networks option, then you need to make sure **Microsoft.Storage** service endpoint is enabled on all subnets in which the SAP system VMs exist. This service endpoint is used to enable access from the SAP virtual machine to the managed storage account, to access the scripts that ACSS runs on the VM extension.
...
- Make sure ASCS, Application Server, and Database VMs of the SAP system are in **Running** state.
...
- For successful discovery and registration of the SAP system, ensure there's network connectivity between ASCS, App, and DB VMs. The `ping` command for App instance hostname must be successful from ASCS VM. `ping` for Database hostname must be successful from App server VM.
...
- Windows, SUSE, and RHEL Linux OSs
- HANA, DB2, SQL Server, Oracle, Max DB, and SAP ASE databases
- SAP system with multiple Application Server Instances on a single VM
...
- Multiple SIDs running on same set of VMs. For example, two or more SIDs sharing a single VM for ASCS instance.
...
ACSS deploys a **managed storage account** into your subscription, for each SAP system being registered. When you register your SAP system using Azure portal, PowerShell, or REST API, choose the **network access** setting for the storage account. You can choose either public network access or access from specific virtual networks.

To secure the managed storage account and limit access to only the virtual network that has your SAP VMs, you can choose the network access setting as **Enable access from specific Virtual Networks**. You can learn more about storage account network security in [this documentation](../../storage/common/storage-network-security.md).

> [!IMPORTANT]
> When you limit storage account network access to specific virtual networks, you have to configure **Microsoft.Storage** [service endpoint](../../virtual-network/virtual-network-service-endpoints-overview.md) on all subnets related to the SAP system that you're registering. Without the service endpoint enabled, you can't successfully register the system. Private endpoint on managed storage account isn't currently supported in this scenario.
...
1. On the **Azure Center for SAP solutions** page, select **Register an existing SAP system**.
   ...
   :::image type="content" source="media/register-existing-system/register-button.png" alt-text="A screenshot of the Azure Center for SAP solutions service overview page in the Azure portal. The 'Register existing SAP system already in Azure' option is highlighted." lightbox="media/register-existing-system/register-button.png":::
...
1. On the **Basics** tab of the **Register existing SAP system** page, provide information about the SAP system.
   ...
   1. For **Managed resource group name**, optionally enter a resource group name as per your organization's naming policies. The ACSS service manages the resource group.
   1. For **Managed storage account name**, optionally enter a storage account name as per your organization's naming policies. The ACSS service manages the storage account.
   ...
   1. Select **Review + register** to discover the SAP system and begin the registration process.
      ...
      :::image type="content" source="media/register-existing-system/registration-page.png" alt-text="A screenshot of the Azure Center for SAP solutions registration page highlighting mandatory fields to identify the existing SAP system." lightbox="media/register-existing-system/registration-page.png":::
...
- Once you fix the configuration causing the issue, retry registration using the **Retry** action available on the Virtual Instance for SAP solutions page on Azure portal.
...
1. If the service isn't running, then restart this service. To restart, use the following steps:
...
1. If these steps don't solve the issue, try updating the VM Agent using [this document](/azure/virtual-machines/extensions/update-linux-agent).
1. If the VM agent doesn't exist or needs to be reinstalled, then follow [this documentation](/azure/virtual-machines/extensions/update-linux-agent).
...
**Solution:** To fix the issue, reconfigure the SAP system so that there's only one ASCS instance present for the SID. Perform the following steps:

1. Sign in to the affected server at the OS level as **'sid'adm**.
1. Run `ps -ef | grep sapstartsrv`, and take note of the command line related to the `sapstartsrv` process from the affected instance.
1. Run `sapcontrol -nr <$$> -function StopService`. Run the `ps` command again (see the previous step), and ensure that the `sapstartsrv` process was stopped (<$$> is the number of the affected instance).
1. Access the folder `/usr/sap/'SID'/SYS/global/sapcontrol`.
1. If you list the files with `ls -l`, there's more than one file for the affected server per observation.
1. The name of the files consists of a few numbers separated by the underscore ("**_**") character, and the last field is the hostname of the server related to that particular file.
1. Delete (move or rename if you prefer) all the conflicting files with the command `rm *hostname` (where "hostname" is the actual name of the server).
1. Manually start the `sapstartsrv` process again using the command line from step #2.
