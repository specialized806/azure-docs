...
description: Learn how to quickly get started with SAP Deployment Automation Framework by deploying an example configuration using sample parameter files.
author: kimforss
ms.author: kimforss
...
The SAP automation deployment framework can use an Azure App Service for configuring the tfvars parameter files.
...
Open PowerShell and copy the following script and update the parameters to match your environment.
...
You can run SAP Deployment Automation Framework from a VM in Azure. The following steps describe how to create the environment.

> [!IMPORTANT]
> Ensure that the VM is using either a system-assigned or user-assigned identity with permissions on the subscription to create resources.

Ensure the VM has the following prerequisites installed:
...
You can install the prerequisites on an Ubuntu VM by using the following command:
