...
App Configuration collects the same kinds of monitoring data as other Azure resources that are described in [Monitoring data from Azure resources](/azure/azure-monitor/essentials/monitor-azure-resource#monitoring-data). See [Data reference](#data-reference) for detailed information on the metrics and logs created by App Configuration.

## Metrics
Metrics are collected automatically for your App Configuration store and are available immediately—no more settings required.

### Metrics overview page in the Azure portal
The **Overview** page in the Azure portal includes a brief view of the resource usage,  such as the total number of requests, number of throttled requests, and request duration per configuration store.
...
For a list of the platform metrics collected for App Configuration, see [Metrics reference](#metrics-reference). For reference, you can also see a list of [all resource metrics supported in Azure Monitor](/azure/azure-monitor/essentials/metrics-supported).

## Logs
Azure Monitor logs include activity logs and resource logs.

### Activity logs
Activity logs provide insight into subscription-level events and show management plane activities (create, update, delete operations on your App Configuration resource). However, they don't capture data plane operations like key-value reads and writes.
...
- **Audit logs**: Capture write operations (create, update, delete) on your data plane resources, such as key-values. Audit logs don't include data plane read operations but include management plane operations. Entries are unaggregated. Query them in the **AACAudit** table.
- **HTTP request logs**: Capture both read and write operations on your data plane resources, such as key-values. Operations are aggregated for better performance and to reduce log volume. Aggregation is based on HTTP method and status code. Each log entry may represent multiple similar operations within a certain time window. Query them in the **AACHttpRequest** table.
...
Resource Logs (including audit logs and HTTP request logs) aren't collected and stored until you create a diagnostic setting and route them to one or more locations. For example, to view logs and metrics for a configuration store in near real-time in Azure Monitor, collect the resource logs in a Log Analytics workspace. If you don't already have one, create a [Log Analytics workspace](/azure/azure-monitor/logs/quick-create-workspace) and follow these steps to create and enable a diagnostic setting.
...
When you create a diagnostic setting, you specify which categories of logs to collect. For further information on the categories of logs for App Configuration, reference [Resource logs reference](#resource-logs-reference).

#### Permission for Log Analytics
To control access to logs stored in a Log Analytics workspace, assign appropriate Azure role-based access control (RBAC) roles on that workspace. The following built-in roles provide different levels of access:
- **Log Analytics Reader**: Can view and query all log data
- **Log Analytics Contributor**: Can view and query all log data, and perform advanced operations

For detailed information on Log Analytics workspace access control, see [Manage access to Log Analytics workspaces](/azure/azure-monitor/logs/manage-access#built-in-roles).
...
If you have created diagnostic settings for audit logs and HTTP request logs, you can view them in the **Logs** blade.
