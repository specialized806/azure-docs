...
Azure Maps visual aren’t supported in the `Embed for your customers` create‑report scenario when using an embed token generated by the [Generate Embed Token for Create Report in Group API](https://learn.microsoft.com/en-us/rest/api/power-bi/embed-token/reports-generate-token-for-create-in-group).
