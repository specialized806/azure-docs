...
> [!NOTE]
> **Power BI Embedded limitation**: The Azure Maps Power BI visual has a known limitation when used with Power BI Embedded in "Embed for customers" scenarios. Specifically, the visual does not function properly when using an embed token generated by the [Generate Embed Token V2 API](https://learn.microsoft.com/rest/api/power-bi/embed-token/generate-token). Organizations implementing custom embedding solutions should be aware of this constraint when designing their Power BI Embedded applications.
