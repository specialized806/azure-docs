...
Azure Maps [Search service] includes API that offers various capabilities to help developers to search addresses, places, business listings by name or category, and other geographic information. For example, [Search Autocomplete] allows users to search for an address or place.
...
To geobias results toward your user's relevant area, include as many location-specific details as possible. You can do this by providing rich information in the generalized `query` parameter, or by using multiple specific parameters such as `addressLine`, `adminDistrict`, `countryRegion`, `locality`, and `postalCode`.

Additionally, you can specify a location using the `coordinates` parameter, which defines a single point on Earth using a longitude-latitude pair. To define a broader search area, use the `bbox` parameter. A bounding box is a rectangular area defined by two coordinate pairs, the southwest corner (minimum longitude, latitude) and the northeast corner (maximum longitude, latitude), that delineates a geographic region.
