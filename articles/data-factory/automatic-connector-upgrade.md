...
| [Xero](connector-xero.md) | Scenario that doesn't rely on below capability in Xero (version 1.0):<br><br>• Use OAuth 1.0 authentication type. <br>• Use `query`. <br><br>The following Xero tables are supported for automatic upgrade: <br>&nbsp;&nbsp;Accounts, Bank_Transactions, Bank_Transfers, Budgets, Contact_Group_Contacts, Contact_Groups, Credit_Notes, Currencies, Invoices, Items, Journals, Manual_Journals, Organisations, Overpayments, Payments, Projects, Receipts, Tax_Rates, Tracking_Categories, Users.   <br><br>If your pipeline runs on self-hosted integration runtime, it requires SHIR version 5.61 or above. |
