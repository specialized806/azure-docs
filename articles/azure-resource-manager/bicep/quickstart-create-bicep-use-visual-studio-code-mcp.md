...
To set up your environment for Bicep development, see [Install Bicep tools](install.md). After completing those steps, you have [Visual Studio Code](https://code.visualstudio.com/) and the [Bicep extension](https://marketplace.visualstudio.com/items?itemName=ms-azuretools.vscode-bicep) version v0.40.2 or later installed. You also have either the latest [Azure CLI](/cli/azure/) version or [Azure PowerShell module](/powershell/azure/new-azureps-module-az).
...
- If the status is `Stopped`, select `Bicep`, and then select `Start Server`.

## Configure Bicep MCP server tools

- Create a new Bicep file called `main.bicep`.
- From the `View` menu, select `Chat` to open the Chat pane.

    :::image type="content" source="./media/quickstart-create-bicep-use-visual-studio-code-mcp/vscode-chat.png" alt-text="Screenshot of Visual Studio Code chat pane.":::

    On the bottom of the pane, it shows the current file context is `main.bicep`; the current mode is `Agent`, and the AI model is `GPT-4.1`. 

- Select the `Configure tools` icon. 
- Expand `Bicep` to see the available Bicep MCP server tools. Select all of them if they are not selected, and then select `OK`. 

## Use the MCP server tools

1. In the `Chat` pane, type a prompt similar to `Add a storage account resource with the Bicep best practices`.

    It shows the Bicep MCP tool used is `Get Bicep best-practices`.

1. Hoover your cursor over the code generated, and then select `Apply in Editor` to add the code to `main.bicep`.
1. In the editor, select `Keep` to confirm the insert. The Bicep file is simliar to:
...
1. In the `CHAT` pane, type a prmopt simliar to `Use a unique string for the storage account name.`.
1. Hoover your cursor over the code generated, and then select `Apply in Editor` to add the code to `main.bicep`.
1. In the editor, select `Keep` to confirm the update. 

    :::image type="content" source="./media/quickstart-create-bicep-use-visual-studio-code-mcp/interpolation-unique-string.png" alt-text="Screenshot of applying interpolation and unique string.":::
