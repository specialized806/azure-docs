...
<<<<<<< HEAD
Site Recovery helps you keep your applications up and running in the event of planned or unplanned zone/regional outages. Enabling Site Recovery on your machines at scale through the Azure portal can be challenging. Azure Policy can help you enable replication at scale without resorting to any scripting.
=======
...
>>>>>>> a98f76f4afedeb8887c368511e81956e66f1c5aa
...
- Review the following support matrix for Azure Site Recovery policy support:
  ...
  **Scenario** | **Support statement**
  ...
  Proximity placement groups (PPGs) | Supported. If the source VM is inside a PPG, the policy creates a PPG by appending *-asr* on the source PPG and uses it for failover to the DR/secondary region.
...
> [!NOTE]
> Site Recovery isn't enabled if an unsupported VM is created within the scope of the policy.
...
1. Select **Assignments** on the left side of the Azure Policy page. An assignment is a policy that you assign to run on a specific scope.
...
1. Open the policy definition picker by selecting the ellipses next to **Policy definition**. Search for **disaster recovery** or **site recovery**. You find a built-in policy titled **Configure disaster recovery on virtual machines by enabling replication via Azure Site Recovery**. Select it and select **Select**.
...
1. Select appropriate values for these parameters:
    ...
    - **Source Region**: Enter the source region of the virtual machines for which the policy applies.
    
      >[!NOTE]
      >The policy applies to all the virtual machines that belong to the source region in the scope of the policy. Virtual machines not present in the source region aren't included.

    - **Target Region**: Enter the location where your source virtual machine data is replicated. Site Recovery provides the list of target regions that you can replicate to. If you want to enable zone-to-zone replication within a region, select the same region as the **Source Region** value.
    ...
    - **Recovery Services Vault**: This is the vault in which all the VMs of the scope are protected. The policy can create a new vault on your behalf, if required.
    ...
    - **Target Availability Zone** **(optional)**: Enter the availability zone of the target region where the virtual machine fails over. If some of the virtual machines in your resource group are already in the target availability zone, the policy isn't applied to them in case you're setting up zone-to-zone DR.
    - **Cache storage account** **(optional)**: Azure Site Recovery uses a storage account for caching replicated data in the source region. Select an account of your choice. You can choose the default cache storage account if you don't have any special considerations.
    ...
    - **Tag values** **(optional)**: Use this field to enter a tag value. For example, enter **Production**.
    - **Tag type** **(optional)**: Use tags to include VMs as part of the policy assignment by selecting **Tag type = Inclusion**. This type ensures that only the VMs that have the tag (provided via **Tag name** and **Tag values** fields) are included in the policy assignment. 
...
1. On the **Remediation** tab in the **Assign policy** workflow, select the **Create a Remediation Task** checkbox.
   ...
   Azure Policy creates a [managed identity](../governance/policy/how-to/remediate-resources.md) that has owner permissions to enable Azure Site Recovery for the resources in the scope.
...
After you assign the policy, wait for up to one hour for replication to be enabled. After that, go to the Recovery Services vault that you chose during policy assignment and look for replication jobs. You should be able to find all VMs for which Site Recovery was enabled via policy in this vault.  

If the VMs don't show up in the vault as protected, you can go back to the policy assignment and try to remediate. 

If the VMs show up as noncompliant, it might be because policy evaluation happened before the VM was completely up and running. You can choose to either remediate or wait for up to 24 hours for the policy to evaluate the subscription or resource group and remediate automatically.
