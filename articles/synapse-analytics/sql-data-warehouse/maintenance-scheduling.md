...
> - DW400c and lower could experience multiple brief losses in connectivity at various times during the maintenance window.

## Alerts and monitoring

Azure provides comprehensive insights into the health of your cloud resources, including current and upcoming issues, service-impacting events, planned maintenance, and other changes that could affect availability.
...
- [How to create a service health alert](/azure/service-health/alerts-activity-log-service-notifications-portal#how-to-create-a-service-health-alert).

Remarks:

As part of the configurations you need to set the condition details to match the service that you're using.
...
> [!NOTE]
> A 24-hour advance notification precedes all maintenance events. In the event we're required to deploy a time critical update, advanced notification times could be reduced. This could occur outside an identified maintenance window due to the critical nature of the update.
...
By default, all newly created data warehouse instances have an eight-hour primary and secondary maintenance window applied during deployment. As indicated above, you can change the windows as soon deployment is complete. No maintenance takes place outside the specified maintenance windows without prior notification.

To view the maintenance schedule that has been applied to your Synapse SQL pool, complete the following steps:

1. Sign in to the [Azure portal](https://portal.azure.com/).
1. Select the Synapse SQL pool that you want to view.
1. The selected Synapse SQL pool opens on the overview window. The maintenance schedule that's applied to the data warehouse appears below **Maintenance schedule**.

:::image type="content" source="./media/maintenance-scheduling/clear-overview-blade.PNG" alt-text="Screenshot of the overview window showing the maintenance schedule.":::

## Skip or change maintenance schedule

To ensure compliance with latest security requirements, we're unable to accommodate requests to skip or delay these updates. However, you might have some options to adjust your maintenance window if you're using DW500c and higher
data warehouse tiers within the current cycle depending on your situation:
- If you receive a pending notification for maintenance, and you need more time to finish your jobs or notify your team, you can change the window start time as long as you do so before the beginning of your defined maintenance window. This shifts your window forward in time within the cycle.
...
- Although we do require a minimum window of 3 hours, under typical conditions this operation completes in well under 30 minutes. However, it's important to note that in some cases, it can take longer. For instance, if there are active transactions when the maintenance begins, they'll be canceled and rolled back, potentially causing delays in restoring the online service. To prevent this situation, we recommend ensuring that there are no long-running transactions active at the start of your maintenance window.

> [!NOTE]
> - If you change the window to a start time before the actual present time, maintenance is triggered immediately and if there are active transactions when the maintenance starts, they'll be aborted and rolled back.
> - After the pause and resume operation is completed to initiate the maintenance, instead of receiving a notification confirming the completion of the maintenance, you'll be notified that it has been canceled.
> - In case you're using DW400c or lower, although you're able to change the maintenance schedule, it will not be adhered to since it's a lower performance level. As mentioned earlier, these data warehouse tiers can undergo maintenance at any time with the maintenance cycle.
...
1. Sign in to the [Azure portal](https://portal.azure.com/).
1. Select the Synapse SQL pool that you want to update. The page opens on the overview window.
Open the page for maintenance schedule settings by selecting the **Maintenance Schedule summary** link on the overview window. Or, select the **Maintenance Schedule** option on the left-side resource menu.

    :::image type="content" source="./media/maintenance-scheduling/maintenance-change-option.png" alt-text="Screenshot of the overview window showing maintenance schedule options.":::
...
During preview, some regions might not yet support the full set of available **Day** options.
   ...
   :::image type="content" source="./media/maintenance-scheduling/maintenance-settings-page.png" alt-text="Screenshot of the maintenance settings window.":::
   
1. Choose your preferred primary and secondary maintenance windows by using the drop-down list boxes:
   ...
   The **Schedule summary** area at the bottom of the window is updated based on the values that you selected.
...
1. Select **Save**. A message appears, confirming that your new schedule is now active.
   ...
   :::image type="content" source="./media/maintenance-scheduling/maintenance-not-active-toast.png" alt-text="Screenshot of the message indicating maintenance scheduling isn't active in the selected region.":::
...
### We were notified about an upcoming dedicated SQL pool scheduled maintenance with tracking ID 0000-000, but it was later canceled or rescheduled. What prompted the cancellation or rescheduling of the maintenance?
